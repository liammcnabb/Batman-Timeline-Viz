...
async function fetchIssue(issueNumber: number): Promise<IssueData> {
  try {
    ...
    throw new Error(`Could not fetch issue ${issueNumber}: ${error.message}`);
  }
}
...
import type { IssueData, ProcessedVillain, TimelineData } from "../types";
```
...
<div class="villain" data-villain-id="green-goblin">Green Goblin</div>
```
...
describe("normalizeVillainName", () => {
  ...
  it("should trim whitespace", () => {
    expect(normalizeVillainName("  Doctor Octopus  ")).toBe("Doctor Octopus");
  });
});
```
